...
### 滑动球功能
- **圆形手势滚动**: 顺时针画圈模拟鼠标滚轮向下 (Shift+Page Down)，逆时针模拟滚轮向上 (Shift+Page Up)
- **透明度可调**: 长按滑动球弹出透明度选择对话框 (10%-100%)
- **位置保存**: 滑动球位置自动保存和恢复
- **可拖动**: 自由拖动定位

### 功能键简化
- 快捷键按钮简化为 F1、F2、F3
- 长按可选择预设功能键 (F1-F8, Ctrl+C/D/Z, Tab)
- 保留 Esc 和 Del 按钮

### 快速输入功能
- 右上角新增3个快速输入按钮 (快1、快2、快3)
- 点击将预设内容插入输入框，不自动执行
- 长按可编辑标签和内容
- 默认预设: `docker ps -a`, `kubectl get pods`, `htop`
- 背景半透明，不遮挡终端内容
